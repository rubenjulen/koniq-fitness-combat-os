 ...
 *  - Lokaal/demo: ingebedde PGlite (OS tmpdir), geen setup nodig.
 * Beide gebruiken dezelfde parameterized SQL ($1, $2, ...) — de queries wijzigen niet.
 * Zet SEED_ON_BOOT=false om automatisch seeden in productie uit te schakelen.
 */
...
declare global {
  ...
  var __koniq_fit_pg: { db: Adapter; ready: Promise<void> } | undefined;
}

function makePglite(): Adapter {
  // Buiten OneDrive houden: OneDrive synct/lockt de PGlite-bestanden → corruptie.
  const dataDir = process.env.PGLITE_DIR || join(tmpdir(), "koniq-fitness-pgdata");
...
function bootstrap() {
  ...
  const ready = (async () => {
    ...
    if (process.env.SEED_ON_BOOT !== "false") await seedDatabase(db);
  })();
  return { db, ready };
}

function getPg() {
  return globalThis.__koniq_fit_pg ?? (globalThis.__koniq_fit_pg = bootstrap());
}
